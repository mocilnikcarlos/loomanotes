...
export function FileHoverBanner({ file, onClick }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="absolute bottom-0 left-0 right-0 flex items-center gap-3 px-4 py-3 text-xs text-foreground backdrop-blur-sm cursor-pointer opacity-0 translate-y-2 transition-all duration-300 ease-out group-hover:opacity-100 group-hover:translate-y-0"
    >
